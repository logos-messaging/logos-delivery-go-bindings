...
//
// It mirrors the Nim MessagingClient, which drives a Waku kernel rather than
// wrapping one: a MessagingClient owns a kernel.Node, and the messaging
// operations are methods on it rather than free functions over a context
// handle:
//
...
//
// The Kernel API runs against the same node, reached through Node. There is no
// second node and no handle to pass around:
//
//	node := client.Node()
