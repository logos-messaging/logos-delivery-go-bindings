...
//
// Every marshalled field is omitted when empty, and that matters: the library
// still accepts a legacy flat configuration blob, and it decides between the
// two shapes by looking for bare top-level keys. A Config that emitted zero
// values would be read as a flat blob and silently ignore the layered
// defaults, so do not add fields here without `omitempty`.
type Config struct {
